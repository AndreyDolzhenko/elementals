...
const GuessSTM: React.FC = () => {
  const [newArrProductSTM, setCopyArrProductSTM] = useState(copyArrProductSTM);
  ...
  function selectionOfBrandsAndStatements() {
    ...
    let nextItem = Math.floor(0 + Math.random() * copyArrProductSTM.length); // Индекс из диапозона элементов copyArrProductSTM.
    ...
    setShowSTM([
      ...
      Object.values(arrProductSTM[nextItem]),
    ]);
    newArrProductSTM.splice(nextItem, 1);
    setCopyArrProductSTM(newArrProductSTM);
